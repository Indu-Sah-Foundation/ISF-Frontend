...
interface Props {
  /**
   * Called once a file is successfully uploaded with the markdown snippet
   * to insert (e.g. `![filename](https://.../blob.jpg)`).
   */
...
// Wraps an editor with drag-and-drop image upload. Files dropped or pasted
// trigger: SAS request -> direct Blob PUT -> markdown insertion via callback.
export function ImageDropzone({ onUploaded, children }: Props) {
  ...
  const upload = useCallback(
    async (file: File) => {
      ...
      try {
        ...
        // 2. PUT file bytes direct to Blob storage
        ...
        setFiles((prev) =>
          prev.map((f) =>
            f.id === id
              ? { ...f, progress: "error", error: err.message || "Upload failed" }
              : f,
          ),
        );
      }
  ...
  const { getRootProps, getInputProps, isDragActive, open } = useDropzone({
    ...
    accept: ACCEPT,
    maxSize: MAX_BYTES,
    noClick: true, // we provide our own click target via `open()`
    noKeyboard: true,
  });
      ...
      {isDragActive && (
        <div className="absolute inset-0 z-20 border-2 border-dashed border-primary bg-primary/5 backdrop-blur-sm flex items-center justify-center pointer-events-none">
          <div className="text-center">
            ...
            <p className="font-display font-bold text-lg">Drop image to upload</p>

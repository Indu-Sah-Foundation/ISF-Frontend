...
export function ArticleEditor({ mode }: Props) {
  ...
  // Cmd/Ctrl-V on a copied screenshot → upload + insert
  ...
  // Auto-suggest slug from title (only when slug is empty in create mode)
    ...
    <div className="max-w-7xl mx-auto px-6 py-12">
      <div className="flex items-end justify-between gap-4 mb-10">
        <div>
          ...
          <h1 className="font-display text-3xl md:text-4xl font-extrabold tracking-tighter mt-2 truncate max-w-2xl">
            {title || "Untitled"}
          </h1>
        </div>
          ...
          {mode.kind === "edit" && (
            <div className="border border-border p-6 bg-card space-y-3">
              ...
              <dl className="space-y-2 font-mono text-xs">
                <Row k="Created" v={new Date(mode.article.created_at).toLocaleString()} />
                <Row k="Updated" v={new Date(mode.article.updated_at).toLocaleString()} />
                <Row
                  k="Published"
                  v={

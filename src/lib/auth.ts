import { useEffect, useState } from "react";
import { getToken, setToken } from "./api";

// Decodes the JWT payload (unverified — we trust it because we just got it
// from our own backend over TLS, and we only use it for UI hints like "show
// admin email"). Real authorization happens server-side on every request.
export interface JwtClaims {
  uid: string;
  email: string;
  role: string;
  exp: number;
  iat: number;
}

export function decodeToken(token: string | null): JwtClaims | null {
  if (!token) return null;
  try {
    const parts = token.split(".");
    if (parts.length !== 3) return null;
    const payload = JSON.parse(
      atob(parts[1].replace(/-/g, "+").replace(/_/g, "/")),
    );
    return payload as JwtClaims;
  } catch {
    return null;
  }
}

export function isExpired(claims: JwtClaims | null): boolean {
  if (!claims) return true;
  return Date.now() / 1000 >= claims.exp;
}

export function useAuth() {
  const [claims, setClaims] = useState<JwtClaims | null>(() =>
    decodeToken(getToken()),
  );

  useEffect(() => {
    const onStorage = () => setClaims(decodeToken(getToken()));
    window.addEventListener("storage", onStorage);
    return () => window.removeEventListener("storage", onStorage);
  }, []);

  const refresh = () => setClaims(decodeToken(getToken()));

  const logout = () => {
    setToken(null);
    setClaims(null);
  };

  return {
    claims,
    isAuthenticated: !!claims && !isExpired(claims),
    isAdmin: !!claims && !isExpired(claims) && claims.role === "admin",
    refresh,
    logout,
  };
}

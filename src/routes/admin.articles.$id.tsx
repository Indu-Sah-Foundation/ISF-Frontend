...
function EditWrapper() {
  ...
  // slug, so we look it up from the list cache and fall back to fetching it.
  const list = useQuery({
  ...
  // Refetch by slug to get the freshest copy (cache may be stale)
    ...
    return (
      <div className="max-w-7xl mx-auto px-6 py-12">
        <div className="space-y-4">
          ...
          <div className="h-96 bg-muted animate-pulse rounded mt-8" />
        </div>
      </div>
    );
  }
  ...
  if (fresh.data) {
    return <ArticleEditor mode={{ kind: "edit", article: fresh.data }} />;
  }
  return <ArticleEditor mode={{ kind: "edit", article }} />;
}

    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-10 py-10 sm:py-12">
      ...
      {donations.data && (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 mb-10">
          ...
          />
          <Stat label="Filter" value={status || "All statuses"} />
        ...
        <div className="border border-border bg-card overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-muted/40 font-mono text-[10px] uppercase tracking-[0.18em] text-muted-foreground">
              <tr>
                <th className="text-left px-4 sm:px-6 py-4">When</th>
                <th className="text-left px-4 sm:px-6 py-4">Donor</th>
                <th className="text-left px-4 sm:px-6 py-4">Status</th>
                <th className="text-right px-4 sm:px-6 py-4">Amount</th>
            ...
            <tbody className="divide-y divide-border">
              {donations.data.items.map((d) => (
                <tr key={d.id} className="hover:bg-muted/30 transition-colors">
                  <td className="px-4 sm:px-6 py-4 font-mono text-xs text-muted-foreground whitespace-nowrap">
                  ...
                  <td className="px-4 sm:px-6 py-4 text-right font-display font-bold whitespace-nowrap">
                    ${(d.amount_cents / 100).toFixed(2)}
                  </td>
                  <td className="px-4 sm:px-6 py-4 font-mono text-[10px] text-muted-foreground hidden md:table-cell">
...
function Stat({ label, value }: { label: string; value: string }) {
  return (
    <div className="border border-border p-4 sm:p-5 bg-card">
      ...
      <div className="mt-2 font-display text-xl sm:text-2xl font-extrabold tracking-tighter truncate">
        {value}
      </div>
    </div>
  );
